...
/**
 * L'accueil du site : le catalogue pris par le lieu.
 *
 * `/formations` répond à « quelle formation, et où se donne-t-elle ? ». Cette
 * page-ci répond à la question inverse, celle qu'on se pose avec un client au
 * téléphone : « qu'est-ce qui se donne autour de cette entreprise ? ». C'est
 * la question la plus fréquente, donc elle passe en premier — l'ancienne
 * page d'accueil n'était qu'un panneau d'affichage avec deux boutons.
 *
 * Mêmes données et mêmes filtres que la liste — le module `@/lib/recherche` les
 * définit une fois pour les deux pages — mais l'entrée est une adresse et la
 * réponse une carte.
 *
...
export default async function AccueilCarte({
  searchParams,
}: {
  ...
  const [domaines, organismes, centresSitues] = await Promise.all([
    ...
    // Le même critère que la carte : avoir des coordonnées, quel que soit le
    // statut de fraîcheur du géocodage.
